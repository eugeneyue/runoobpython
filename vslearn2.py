...
# """
# 非递归
# def binarysearch(list1,seeknum):
#     min,max = 0,len(list1)-1                               # 数字的总和数量是11个 
#     while min <= max:
#         mid = (min + max)//2
#         if list1[mid] == seeknum:
#             return ("目标索引值是{0}".format(mid))
...
#     return "不在列表中"
# if __name__ == "__main__":
#     beflist = [-1, 0, 1, 4, 5, 12, 13, 15, 32, 35, 37]
#     search = int(input("请输入要查找的数字："))
#     print(binarysearch(beflist,search))
# 递归
# def binarysearch(list1,seeknum):
#     min,max = 0,len(list1)-1
#     while min <= max:
#         mid = (min + max)//2
#         if list1[mid] == seeknum:
#             return ("目标索引值是{0}".format(mid))
#         elif seeknum < list1[mid]:
#             return binarysearch(list1[:mid-1],seeknum)
#         else:
#             return binarysearch(list1[mid+1:],seeknum)
#     return "不在列表中"
# def manage(list2):
#     if list2 == list2.sort():                      # 升序列表不做处理
#         return list2              
#     elif list2 == list2.sort(reverse=True):        # 降序列表
